...
class WorkWeixin():
    ...
    def send_message(self, userid_list, message):
        ac_status, msg = self.get_access_tocken()
        if ac_status:
            try:
                data = {
                ...
                url = '%s?access_token=%s' % (self.send_message_url, msg)
...
def work_weixin_lauth(func):
    ...
    def function(request):
        ...
        request_url = BASE_URL + '/project/ww/confirm_login'
        print request_url
        # 获取 openid
        if not request.user.is_authenticated():
            ...
            if 'code' not in request.GET:
                ...
                if not status:
                    ...
                    return HttpResponseBadRequest('登陆失败' + user_id)
        else:
        ...
        if status:
            if User.objects.filter(username=user_id).exists():
                ...
                u = User(username=user_id, first_name=msg['name'])
                u.save()
        ...
        if status:
            ...
            if not User.objects.filter(username=userid).exists():
                ...
                if status:
                    ...
                    auth.login(request, u)
                    if state == 'mobile':
                        r = HttpResponseRedirect('/#/mobile/')
                    else:
                        r = HttpResponseRedirect('/#/')
                    r.set_cookie('username', urlquote(u.username), max_age=720000)
                    r.set_cookie('chinese_name', urlquote(u.first_name), max_age=720000)
                    return r
                else:
            ...
            return HttpResponseBadRequest(
                json.dumps({'msg': 'connect  wechat server  failed ,pls try again later .获取微信服务失败，'
                                   '请稍后重试:{}'.format(msg)}))
    else:
...
@work_weixin_lauth
@require_http_methods(['GET'])
def mobile_index(request, userid=None):
